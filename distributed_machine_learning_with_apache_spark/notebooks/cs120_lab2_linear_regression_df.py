# Databricks notebook source exported at Sun, 7 Aug 2016 23:19:11 UTC
# MAGIC %md
...
def parse_points(df):
    ...
    return (df.select((sql_functions.split(df.value, ',')).alias('value'))
            .rdd
            .map(lambda row: LabeledPoint( float(row['value'][0]), list(row['value'][1:])))
            .toDF() )
    
         

parsed_points_df = parse_points(raw_data_df)
first_point_features = parsed_points_df.select('features').first().features
first_point_label = parsed_points_df.select('label').first().label
print first_point_features, first_point_label

print type(first_point_features)
...
# TODO: Replace <FILL IN> with appropriate code
preds_and_labels_train = parsed_train_data_df.map(lambda x: (average_train_year, x.label)).collect()
...
preds_and_labels_val = parsed_val_data_df.map(lambda x: (average_train_year, x.label)).collect()
...
preds_and_labels_test = parsed_test_data_df.map(lambda x: (average_train_year, x.label)).collect()
...
def get_labeled_prediction(weights, observation):
    ...
    prediction = observation.features.dot(weights)
    return (float(prediction), float(observation.label))
...
def two_way_interactions(lp):
    ...
    """
    merged_lp_features = []
    interactions = list( ( itertools.product(lp.features, repeat = 2)))
    for (i,j) in interactions:
      merged_lp_features.append(i*j)
      
    return LabeledPoint(lp.label, np.hstack((lp.features, merged_lp_features)))
  
print two_way_interactions(LabeledPoint(0.0, [2, 3]))

print type(parsed_train_data_df)
print parsed_train_data_df.label

# Transform the existing train, validation, and test sets to include two-way interactions.
# Remember to convert them back to DataFrames at the end.
#train_data_interact_df = two_way_interactions(LabeledPoint(parsed_train_data_df.label, parsed_train_data_df.features)).toDF()
