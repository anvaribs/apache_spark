# Databricks notebook source exported at Wed, 10 Aug 2016 03:18:19 UTC
# MAGIC %md
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """
    index_value_pair_list = []
    for i in range(0, len(raw_feats)):
      index_value_pair_list.append((ohe_dict_broadcast.value[raw_feats[i]], 1))
    
    final_sparse_vector = SparseVector(num_ohe_feats, index_value_pair_list)
    return final_sparse_vector
...
def parse_point(point):
    ...
    """
    featureID_value_pair_list = []
    j = -1
    for i in point.split(','):
      featureID_value_pair_list.append((j, i))
      j += 1
    featureID_value_pair_list.pop(0)
    return featureID_value_pair_list
...
raw_train_df.select(split(raw_train_df.text, ',').getItem(0).cast("double").alias('label'), parse_point_udf(raw_train_df.text).alias('features')).show()
...
def parse_raw_df(raw_df):
    ...
    """
    return (raw_df
            .select(split('text', ',').getItem(0).cast("double").alias('label'), parse_point_udf('text').alias('feature'))
            .cache())
...
# TODO: Replace <FILL IN> with appropriate code
ctr_ohe_dict = create_one_hot_dict(parsed_train_df.withColumnRenamed('feature', 'features'))
...
ohe_train_df = (parsed_train_df
                .select(parsed_train_df.label, ohe_dict_udf(parsed_train_df.feature).alias('features'))
                .cache())
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    index_value_pair_list = []
    for i in range(0, len(raw_feats)):
      if ohe_dict_broadcast.value.has_key(raw_feats[i]):
        index_value_pair_list.append((ohe_dict_broadcast.value[raw_feats[i]], 1))
...
ohe_validation_df = (parsed_val_df
                    .select(parsed_val_df.label, ohe_dict_missing_udf(parsed_val_df.feature).alias('features'))
                    .cache())
...
lr = LogisticRegression(featuresCol="features", labelCol="label", predictionCol="prediction", maxIter=max_iter, regParam=reg_param, elasticNetParam=elastic_net_param, standardization=standardization)
...
def add_log_loss(df):
    ...
    """
    new_df = df.select('p', 'label', when(df.p == 1, df.p-epsilon).when(df.p == 0, epsilon).otherwise(df.p).alias("p_modified"))
    return new_df.select('p', 'label', when(new_df['label'] == 1, -log(new_df.p_modified)).otherwise(-log(1-new_df.p_modified)).alias("log_loss"))
...
log_loss_tr_base = (add_log_loss(ohe_train_df.select( lit(class_one_frac_train).alias("p"), "label"))).select(avg("log_loss")).first()[0]
...
log_loss_tr_base = (add_log_loss(ohe_train_df.select( lit(class_one_frac_train)
                                              .alias("p"),   "label"))).select(avg("log_loss")).first()[0]
...
def add_probability(df, model):
    ...
    def get_p(features):
        ...
        raw_prediction = coefficients_broadcast.value.dot(features) + intercept
        # Bound the raw value between 20 and -20
        raw_prediction = clip(raw_prediction, -20, 20)
...
def evaluate_results(df, model, baseline=None):
    ...
    if baseline is not None:
        with_log_loss_df = (add_log_loss(df.select( lit(baseline)
                                                      .alias("p"),   "label")))
    else:
      add_probability_model = lambda df: add_probability(df, model)
      with_probability_df = add_probability_model(df)
...
# TODO: Replace <FILL IN> with appropriate code
log_loss_val_base = evaluate_results(ohe_validation_df,None, ohe_train_df.select(avg("label")).first()[0])
...
parse_raw_df(raw_train_df).show()
...
def add_hashed_features(df):
    ...
    return parsed_df.select("label", tuples_to_hash_features_udf("feature").alias("features"))

hash_train_df = add_hashed_features(raw_train_df)
hash_validation_df = add_hashed_features(raw_validation_df)
hash_test_df = add_hashed_features(raw_test_df)
...
def get_sparsity(df):
    ...
    """
    return df.select(feature_sparsity_udf(df.features).alias('features')).selectExpr("avg(features)").first()[0]
...
lr_hash = LogisticRegression(featuresCol="features", labelCol="label", predictionCol="prediction", maxIter=max_iter, regParam=reg_param, elasticNetParam=elastic_net_param, standardization=standardization)
